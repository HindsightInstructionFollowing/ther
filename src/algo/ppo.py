...
class PPOAlgo(BaseAlgo):
    ...
    def __init__(self, envs, config, logger, device, visualizer=None):
        ...
        super().__init__(envs, acmodel, config, logger, device)
        ...
        self.optimizer = torch.optim.Adam(self.acmodel.parameters(), config["lr"], eps=config["adam_eps"])
        self.batch_num = 0
    ...
    def train(self, n_env_iter, visualizer=None):
        ...
        while num_frames < n_env_iter:
            ...
            for key, value in logs.items():
                self.tf_logger.log(key, np.mean(value))

            # Print logs
            update_end_time = time.time()

            mean_rew = np.mean(logs["return_per_episode"])
            p_loss = logs["policy_loss"]
            v_loss = logs["value_loss"]
            grad_norm = logs["grad_norm"]
            duration = int(time.time() - start_time)

            fps = logs["num_frames"] / (update_end_time - update_start_time)
            str_ = "U {} | F {:07} | FPS {:03.0f} | R {:.2f} | policy loss {:.3f} | value_loss {:.3f} | grad_norm {:.2f} ||| in {}".format(
